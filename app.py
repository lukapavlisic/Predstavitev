...
@app.route('/rezerviraj-termin', methods=['POST'])
def rezerviraj_termin():
    data = request.get_json()
    car_name = data.get('car')
    date = data.get('date')
    time = data.get('time')
    user = session.get('user_id', 'gost')

    if not (car_name and date and time):
        return jsonify({'error': 'Manjkajoči podatki!'}), 400

    rezervacije_table = db.table('rezervacije')
    conflict = rezervacije_table.get((where('car') == car_name) & (where('date') == date) & (where('time') == time))
    if conflict:
        return jsonify({'error': 'Ta termin za pregled tega avta je že zaseden!'}), 409

    rezervacije_table.insert({
        'car': car_name,
        'date': date,
        'time': time,
        'user': user,
    ...
    return jsonify({'message': f'Termin za preventivni pregled avta "{car_name}" uspešno rezerviran na {date} ob {time}.'})

@app.route('/koledar-terminov')
def koledar_terminov():
    car = request.args.get('car')
    rezervacije_table = db.table('rezervacije')
    rezervacije = rezervacije_table.search(where('car') == car) if car else []
    return jsonify({'reservations': [
        {'date': r['date'], 'time': r['time']} for r in rezervacije
    ]})
